...
def _generate_coverage_report(config: CoverAgentWorkflowConfig, output_dir: Path) -> None:
	...
	try:
		...
		branch_rate_avg = sum(float(node.get("branch-rate", "0")) for node in source_nodes) / len(source_nodes)
		...
		with open(report_path, "w", encoding="utf-8") as f:
			...
			f.write("## 结论\n\n")
			if timeline:
				start_cov = timeline[0]["coverage"]
				end_cov = timeline[-1]["coverage"]
				total_gain = end_cov - start_cov
				f.write(f"- 从初始覆盖率 {start_cov:.2f}% 提升到 {end_cov:.2f}%，累计提升 {total_gain:+.2f}%。\n")
			else:
				f.write("- 无法从日志中提取迭代覆盖率变化，建议检查 run.log 是否被正确保存。\n")
...
def run_cover_agent_workflow(config: CoverAgentWorkflowConfig) -> None:
	...
	_move_if_exists(project_root / "run.log", output_dir / "run.log")
	_move_if_exists(test_command_dir / ".coverage", output_dir / ".coverage")
	
	# Handle stored_responses: delete if empty, otherwise move
	stored_responses_src = project_root / "stored_responses"
	if stored_responses_src.exists() and stored_responses_src.is_dir():
		if any(stored_responses_src.iterdir()):
			_move_if_exists(stored_responses_src, output_dir / "stored_responses")
		else:
			shutil.rmtree(stored_responses_src)
